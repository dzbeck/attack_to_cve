...
If a vulnerability is associated with a CWE, the associated CWE definition at [[5]](#5) (use the ID Lookup search) can provide pointers to applicable ATT&CK mappings. More specifically, the "Common Consequences" section can suggest likely impacts. For example, the common consequences given for CWE-190: Integer Overflow are "DOS," "Modify Memory," and "Execute Unauthorized Code." The "Modify Memory" consequence is directly associated with exploiting the overflow; however, "DOS" and "Execute Unauthorized Code" suggest the impact mappings T1499 and T1574, respectively (as shown in the Table 1).
